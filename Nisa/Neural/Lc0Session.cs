...
/// <summary>Thread-safe Leela-style network wrapper with built-in mini-batching.</summary>
internal sealed class Lc0Session : IDisposable
{
    ...
    private readonly string _wdlName;     // ← NEW
    ...
    private sealed record Request(Board Board,
    ...
    public Lc0Session(string modelPath, EngineOptions opt)
    {
        ...
        switch (opt.Backend)
        {
            case GpuBackend.DirectML: TryAppend(() => so.AppendExecutionProvider_DML(0), "DirectML"); break;
            case GpuBackend.Cuda:     TryAppend(() => so.AppendExecutionProvider_CUDA(0), "CUDA");   break;
            default: Console.Error.WriteLine("info string CPU provider selected"); break;
        }
        ...
        foreach (var (k, v) in _sess.OutputMetadata)
            Console.Error.WriteLine($"Output `{k}` dims: {string.Join('×', v.Dimensions)}");
    ...
    // ─── public API ──────────────────────────────────────────────────────────
    public (float value, float[] policy) Evaluate(Board b)
    {
        if (_cache.TryGet(b, out var hit)) return hit;

        var tcs = new TaskCompletionSource<(float, float[])>(
                      TaskCreationOptions.RunContinuationsAsynchronously);
        _queue.Add(new Request(b, tcs), _cts.Token);
        return tcs.Task.GetAwaiter().GetResult(); // block until the worker replies
    }
            ...
            while (!_cts.IsCancellationRequested)
            {
                ...
                for (int i = 0; i < B; i++)
                {
                    var slice = new DenseTensor<float>(
                        ...
                        new[] { 1, _planes, 8, 8 });
                    Encoder.Encode(batch[i].Board, slice);
                }
                ...
                using var results = _sess.Run(
                    ...
                    new[] { _policyName, _wdlName, _valueName }      // request exactly the heads we need
                );
                ...
                var valueFlat  = results.First(r => r.Name == _valueName).AsEnumerable<float>().ToArray(); // optional
                ...
                for (int i = 0; i < B; i++)
                {
                    ...
                    for (int j = 0; j < policyStride; j++) { exp[j] = Math.Exp(logits[j] - m); sum += exp[j]; }
                    var p = new float[policyStride];
                    for (int j = 0; j < policyStride; j++) p[j] = (float)(exp[j] / sum);
                    ...
                    if (wdlFlat.Length >= 3 * (i + 1))
                    {
                        ...
                        v = pw + 0.5f * pd - pl;           // in [-1, +1]
                    }
                    else
                    {
                        v = (float)Math.Tanh(valueFlat[i]); // old 2-head nets
                    }

                    Console.Error.WriteLine($"info string NN eval → v={v:F3}, top1P={p.Max():F3}");

                    _cache.Add(batch[i].Board, (v, p));
    ...
    public void Dispose()
    {
        _cts.Cancel(); _worker.Join();
        _sess.Dispose(); _queue.Dispose(); _cts.Dispose();
    }

    // ─── helpers ────────────────────────────────────────────────────────────
    private static void TryAppend(Action append, string tag)
    {
        try                     { append(); Console.Error.WriteLine($"info string {tag} provider loaded"); }
        catch (Exception ex)    { Console.Error.WriteLine($"info string {tag} unavailable: {ex.Message} (fallback to CPU)"); }
    }
